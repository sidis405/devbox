...
if [[ -n "$CURR_USER_ID" ]] && [[ -n "$CURR_GROUP_ID" ]]; then
    ...
    usermod -u $CURR_USER_ID www-data && echo "gruppo www-data modificato con successo" || exit 1
    groupmod -g $CURR_GROUP_ID www-data && echo "utente www-data modificato con successo" || exit 1
